...
void computeNgram_og32(int channels, int cntr_bits, float buffer[], uint32_t iM[][bit_dim + 1], uint32_t projM_pos[][bit_dim + 1], uint32_t projM_neg[][bit_dim + 1], uint32_t query[bit_dim + 1]){
/*************************************************************************
...
**************************************************************************/
    ...
    for(int i = 0; i < bit_dim + 1; i++){
        ...
        for(int z = 31; z >= 0; z--){
            ...
            if (numberOfSetBits32(majority) > (channels/2)) query[i] = query[i] | ( 1 << z ) ;
